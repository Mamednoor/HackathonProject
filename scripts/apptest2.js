...
function handlePreload() {
    ...
    // Preload our "sky".
    this.load.image( 'starfield', 'assets/Background/startfield22.jpg' );
    // Preload our player.
    this.load.image( 'ship', 'assets/Characters/goat_50px.png' );
    // Preload the enemy image.
    this.load.spritesheet( 'invader', 'assets/Characters/chicken_good.png', {
        frameWidth: 50,
        frameHeight: 60
    });
    ...
    this.load.image( 'enemyBullet', 'assets/Miscelenous/egg_40px.png' );
...
function handleCreate() {
    ...
    this.add.text(
        this.sys.canvas.width - 185,    // From the right. // angle de rotation de la vie
        15,                             // From the top.
        'LIVES:'
    );
...
function handleUpdate( time ) {
    ...
    if ( cursors.left.isDown ) {
        player.setVelocityX( -200 );  // touche dirrectionnel gauche
    }
    else if ( cursors.right.isDown ) {
        player.setVelocityX( 200 ); // touche dirrectionnel droite
    }
    else {
